...
async def run_installation_process(task_id: str, request: InstallationRequest):
    ...
    try:
        ...
        # If no module selected, abort early
        if not request.install_bdpack and not request.install_ecm:
            await handle_failure("No module selected for installation. Set install_bdpack and/or install_ecm.")
            return

        if request.install_bdpack:
            ...
            await append_output(task_id, "[INFO] Skipping BD Pack as requested")
